...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser) -> None:
        parser.add_argument("--email", default="", help="Email for the trial (optional; anonymous if omitted).")
        parser.add_argument("--force", action="store_true", help="Provision even if credentials already exist.")

    def handle(self, *args, **options) -> None:
        storage = DjangoConfigStorage()
        if storage.load() is not None and not options["force"]:
            self.stdout.write(self.style.WARNING("Amazee credentials already stored (use --force to re-provision)."))
            return
        client = AmazeeClient()
