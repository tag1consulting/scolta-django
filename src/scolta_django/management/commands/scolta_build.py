...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser) -> None:
        parser.add_argument("--force", action="store_true", help="Bypass the token cache and fingerprint; full rebuild.")
        parser.add_argument("--sync", action="store_true", help="Run synchronously (the default for this command).")
        parser.add_argument("--incremental", action="store_true", help="Only process tracked changed content.")
        parser.add_argument("--resume", action="store_true", help="Resume an interrupted Python index build.")
        parser.add_argument("--restart", action="store_true", help="Discard interrupted state and restart.")
        parser.add_argument("--memory-budget", dest="memory_budget", default=None, help="conservative|balanced|aggressive|256M|1G")
        parser.add_argument("--chunk-size", dest="chunk_size", type=int, default=None, help="Pages per chunk.")
    ...
    def handle(self, *args, **options) -> None:
        ...
        intent = BuildIntentFactory.from_flags(
            options["resume"], options["restart"], total, budget
        )
